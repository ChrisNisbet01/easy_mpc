...
{
    epc_parser_t * p = epc_char_range(NULL, 'a', 'z');
    session = epc_parse_input(p, NULL);
...
TEST(TerminalParsersNew, Int_MatchesPositiveInteger)
{
    ...
    session = epc_parse_input(p, "12345abc");
...
{
    epc_parser_t * p = epc_int(NULL);
    session = epc_parse_input(p, "-6789xyz");
...
{
    epc_parser_t * p = epc_int(NULL);
    session = epc_parse_input(p, "0def");
...
TEST(TerminalParsersNew, HexDigit_MatchesDigit)
{
    ...
    session = epc_parse_input(p, "5abc");
...
{
    epc_parser_t * p = epc_hex_digit(NULL);
    session = epc_parse_input(p, "cdef");
...
{
    epc_parser_t * p = epc_hex_digit(NULL);
    session = epc_parse_input(p, "ABCE");
...
TEST(TerminalParsersNew, OneOf_MatchesCharInSet)
{
    ...
    session = epc_parse_input(p, "bdef");
...
TEST(TerminalParsersNew, OneOf_MatchesFirstCharInSet)
{
    ...
    session = epc_parse_input(p, "1xyz");
...
TEST(TerminalParsersNew, OneOf_MatchesLastCharInSet)
{
    ...
    session = epc_parse_input(p, "zabc");
...
{
    epc_parser_t * p = epc_one_of(NULL, "abc");
    session = epc_parse_input(p, "dxyz");
...
TEST(TerminalParsersNew, CppComment_MatchesSimpleComment)
{
    ...
    session = epc_parse_input(p, "// A simple comment\nNext line");
...
{
    epc_parser_t * p = epc_cpp_comment_l(list, NULL);
    session = epc_parse_input(p, "// Comment at EOF");
...
{
    epc_parser_t * p = epc_cpp_comment_l(list, NULL);
    session = epc_parse_input(p, "A regular line\n");
...
{
    epc_parser_t * p = epc_cpp_comment_l(list, NULL);
    session = epc_parse_input(p, "/ A single slash comment\n");
    check_failure("Expected '//'");
}
...
TEST(TerminalParsersNew, CComment_MatchesSimpleComment)
{
    ...
    session = epc_parse_input(p, "/* This is a C comment */ Next code");
...
{
    epc_parser_t * p = epc_c_comment_l(list, NULL);
    session = epc_parse_input(p, "/* Multi\nline\ncomment */ After");
...
{
    epc_parser_t * p = epc_c_comment_l(list, NULL);
    session = epc_parse_input(p, "/* Comment * with * stars */ End");
...
{
    epc_parser_t * p = epc_c_comment_l(list, NULL);
    session = epc_parse_input(p, "/**/Something else");
...
{
    epc_parser_t * p = epc_c_comment_l(list, NULL);
    session = epc_parse_input(p, "/* Unterminated comment");
...
{
    epc_parser_t * p = epc_c_comment_l(list, NULL);
    session = epc_parse_input(p, "Not a comment */");
    check_failure("Expected \"/*\"");
}
