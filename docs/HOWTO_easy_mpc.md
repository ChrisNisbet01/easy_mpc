...
`epc_passthru` is a unary combinator that returns the result of its single child parser directly. It effectively makes itself transparent in the Concrete Parse Tree (CPT). It's useful for grouping or renaming a parser's result without adding an extra layer in the CPT.

```c
// Assuming p_number parses an integer, p_term_passthru will directly produce p_number's CPT node.
epc_parser_t* p_number = epc_int_l(list, "integer");
epc_parser_t* p_term_passthru = epc_passthru_l(list, "passthru_term", p_number);
// If input is "123", p_term_passthru succeeds and its result will be the epc_cpt_node_t generated by epc_int (tag "integer", content "123").
// No "passthru" node will appear in the CPT.
```
...
5.  **Grammar Definition**:
    ...
    *   Define combinator parsers using `epc_and`, `epc_or`, `epc_plus`, `epc_many`, `epc_passthru`.
